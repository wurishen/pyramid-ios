...
│   │   └── SessionDetailView（单会话配置，Sheet，只读展示当前角色）
│   ├── QuickSwitcherView（长按 Tab 触发，圆形网格快速切换会话；顶栏「选择角色卡」可建新窗）
...
- 聊天页工具栏可打开会话列表
- 会话列表可创建/删除/置顶会话，点击进入会话详情
- 会话详情只读展示当前角色绑定，可配置预设、世界书绑定、系统提示词
- 角色绑定在新建时确定，会话内不可更换；想换角色请到角色卡 Tab 另开新窗
...
| 助手消息气泡 | **不显示任何角色头像**（角色信息只在会话列表/快速切换器里，避免重复） |
| 会话列表每行 | 会话角色头像或默认首字母 |
| 会话快速切换器 | 顶部长条栏 = 当前角色头像（居中）+ 右侧「选择角色卡」按钮（建新窗）；下方为角色头像圆形气泡网格，气泡下方显示会话标签 |
| 角色卡 Tab 行 | 点头像直接 `createSession(character:)` 并切到聊天 Tab；右键菜单「开新聊天窗 / 编辑」 |
...
- 入口一（聊天 Tab 长按弹出 `QuickSwitcherView`）：顶栏「选择角色卡」按钮 → `CharacterPickerSheet` → 选中后若该角色已绑定到其他会话，弹 alert「打开已有 / 仍要新建 / 取消」；否则直接 `ChatStore.createSession(character:)` 新建并自动选中。
- 入口二（角色卡 Tab `CharacterListView`）：点行或右键「开新聊天窗」→ `ChatStore.createSession(character:)` + `onStartChat` 切到聊天 Tab。
- 入口三（会话列表 `SessionListView`）：右上「新建空白」按钮 → `ChatStore.createSession()`，不绑角色。
- 约束：**角色绑定在新建时确定**。一旦会话创建成功，其 `characterId` 不可再在会话内切换；想换角色请到角色卡 Tab 另开新会话，或在会话列表「详情」查看当前绑定。
