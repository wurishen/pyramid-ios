...
- **上下文 / 界面 / 关于 / 备份**：上下文裁剪策略三选一（**不裁剪 / 最近 N 条消息 / 最近 C 字符**，当前用户消息始终保留），字符数估算 + 超出提示；头像 / 楼层号 / 用户消息 ID 等界面开关；关于页含项目说明与 Base URL 提示；备份页支持导出 / 分享 / 导入（合并 or 覆盖），按 id 去重入库。**没有「清空全部会话与设置」的入口**——要清空可用「导入覆盖」功能。
